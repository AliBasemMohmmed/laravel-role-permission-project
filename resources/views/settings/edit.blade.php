        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card shadow">
                    <div class="card-header bg-primary text-white">Edit Settings</div>
                    <div class="card-body">
                        <form action="{{ route('settings.update', $setting->user_id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <select multiple class="form-control" id="working_days" name="working_days[]">
                                    @foreach(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] as $day)
                                        <option value="{{ $day }}" {{ in_array($day, old('working_days', $setting->working_days ?? [])) ? 'selected' : '' }}>
                                            {{ $day }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select multiple class="form-control" id="vacation_days" name="vacation_days[]">
                                    @foreach(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] as $day)
                                        <option value="{{ $day }}" {{ in_array($day, old('vacation_days', $setting->vacation_days ?? [])) ? 'selected' : '' }}>
                                            {{ $day }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" id="daily_patient_limit" name="daily_patient_limit" value="{{ old('daily_patient_limit', $setting->daily_patient_limit) }}" required>
